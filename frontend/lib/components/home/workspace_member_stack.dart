...
class _WorkspaceMembersDialogState extends State<WorkspaceMembersDialog> {
  ...
  Future<void> _fetchMembers() async {
    final data = await _service.getWorkspaceMembers(widget.workspaceId);
    if (mounted) {
      setState(() {
        _members = data;
        _isLoading = false;
...
class _WorkspaceMemberStackState extends State<WorkspaceMemberStack> {
  ...
  Widget build(BuildContext context) {
    ...
    final int displayCount = _members.length > maxDisplay
        ? maxDisplay
        : _members.length;
    final int extraCount = _members.length > maxDisplay
        ? _members.length - maxDisplay
        : 0;
        ...
        for (int i = 0; i < displayCount; i++)
          Align(
            ...
            child: Container(
              ...
              child: CircleAvatar(
                radius: 14,
                backgroundColor: _getAvatarColor(_members[i].email),
                child: Text(
                  _members[i].email.isNotEmpty
